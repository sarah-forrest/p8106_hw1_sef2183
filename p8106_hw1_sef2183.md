Homework 1: Predicting the Sale Price of a House Using Characteristics
================
Sarah Forrest
2/8/2023
...
The best lambda (λ) value resulting in the minimum MSE according to the
caret package is 63.53019.
...
When the 1 standard error (1SE) rule is applied, there are 36 predictors
included in the model (not including the intercept). The following
coefficients were removed from the model because they had values of 0:
second floor square feet (Second_Flr_SF), “good” fireplace quality
...
The best alpha (α) value using the minimum MSE rule according to the
caret package is 0.65. The best lambda (λ) value using the minimum MSE
rule according to the caret package is 7.389056.
...
When the 1SE rule is applied, the The best alpha (α) value using the 1SE
rule is 0. The best lambda (λ) value using the 1SE rule is 7.389056.
...
426,357,707. The root-mean-square test error (RMSE) for the elastic
netmodel is 20,648.43.
...
440,217,938. The root-mean-square test error (RMSE) for the partial
least squares model is 20981.37.
